...
- [ ] **T1-04 — 前端欢迎对话页面（引导式画像构建）**
  ...
  - 预估工作量：大（>3天）
  - 前置依赖：T0-07, T1-03
  - 简要说明：实现 `WelcomeView.vue`，包含：欢迎动画（AI 学伴角色出场 + 渐变背景 + 微动效）、对话气泡逐步出现（打字机效果）、进度条显示对话进度（5-8 轮）、画像摘要卡片展示 + 确认/修改按钮。使用 SSE 流式接收 AI 回复，调用 `/ai/v1/chat/welcome` 端点
...
- [ ] **T2-02 — LangGraph StateGraph 定义与工作流骨架**
  ...
  - 预估工作量：中（1~3天）
  - 前置依赖：T0-03
  - 简要说明：实现 `ZhiBanState` TypedDict（扩展支持 9 Agent 状态流转），搭建 `build_chat_graph()` 工作流骨架（9 个 Agent 节点占位 + 条件边路由），实现 `with_retry` 重试装饰器
